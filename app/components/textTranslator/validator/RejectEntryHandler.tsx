...
export const HandleReject = async ({ entry, session, sessionUpdate, d, setTriggerFetch }: HandleRejectProps) => {
    ...
    try {
        ...
        const { score, ..._userWithoutScore } = updatedUser;
        //console.log(userWithoutScore, updatedUser);
        sessionUpdate({ user: updatedUser });
        toast.success(`${d?.validator.success_validation.text_before_link}${' '}${validationScore}${' '}${d?.validator.success_validation.text_after_link}`);
    } catch (error) {
        //console.log(error);
